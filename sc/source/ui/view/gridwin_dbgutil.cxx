...
void dumpScDrawObjData(const ScGridWindow& rWindow, const ScDrawObjData& rData, MapUnit eMapUnit)
{
    ...
    Point aStartOffsetPixel = rWindow.convertTo<vcl::WindowPoint>(vcl::LogicPoint(rStartOffset), MapMode(eMapUnit));
    std::cout << "  Start: " << rData.maStart << ", Offset: " << aStartOffsetPixel << std::endl;

    const Point& rEndOffset = rData.maEndOffset;
    Point aEndOffsetPixel = rWindow.convertTo<vcl::WindowPoint>(vcl::LogicPoint(rEndOffset), MapMode(eMapUnit));
...
void ScGridWindow::dumpColumnInformationPixel()
{
    ...
    for (SCCOL nCol = 0; nCol <= 20; ++nCol)
    {
        ...
        tools::Long nPixel = convertTo<vcl::WindowPoint>(vcl::LogicPoint(Point(nWidth, 0)), MapMode(MapUnit::MapTwip)).getX();
...
void ScGridWindow::dumpGraphicInformation()
{
    ...
    for (sal_uInt16 nPage = 0; nPage < nPageCount; ++nPage)
    {
        ...
        for (size_t nObj = 0; nObj < nObjCount; ++nObj)
        {
            ...
            tools::Rectangle aRect = convertTo<vcl::WindowRect>(vcl::LogicRect(rRect), MapMode(pDrawLayer->GetScaleUnit()));

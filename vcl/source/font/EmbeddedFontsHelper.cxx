...
bool EmbeddedFontsHelper::addEmbeddedFont(const uno::Reference<io::XInputStream>& stream,
                                          ...
                                          std::vector<unsigned char> const& key, bool eot)
{
    ...
    for (;;)
    {
        ...
        for (sal_uInt64 pos = 0; pos < read && keyPos < key.size(); ++pos)
    ...
    if (eot)
    {
        ...
        libeot::EOTError uncompressError = libeot::EOT2ttf_buffer(
            reinterpret_cast<unsigned char*>(fontData.data()), fontData.size(), &eotMetadata,
            &nakedPointerToUncompressedFont, &uncompressedFontSize);
        std::shared_ptr<unsigned char> uncompressedFont(nakedPointerToUncompressedFont,
        ...
        {
            sal_uInt64 written;
            if (file.write(uncompressedFont.get() + writtenTotal,
                           uncompressedFontSize - writtenTotal, written)
                != osl::File::E_None)
            {
    ...
    if (!eot)
    {
        sufficientFontRights
            = sufficientTTFRights(fontData.data(), fontData.size(), FontRights::EditingAllowed);
    }
...
{
    UpdateFontsGuard() { OutputDevice::ImplClearAllFontData(true); }

    ~UpdateFontsGuard() { OutputDevice::ImplRefreshAllFontData(true); }
};
}
...
OUString EmbeddedFontsHelper::fontFileUrl(std::u16string_view familyName, tools::FontFamily family,
                                          ...
                                          FontRights rights)
{
    ...
    OUString filename = OUString::Concat(familyName) + "_" + OUString::number(family) + "_"
                        ...
                        + OUString::number(pitch) + ".ttf"; // TODO is it always ttf?
    ...
    for (int i = 0; i < fontInfo->Count(); ++i)
    {
        ...
        if (f->GetFamilyName() == familyName)
        {
            ...
            if ((f->GetFamilyType() == tools::FAMILY_DONTKNOW || family == tools::FAMILY_DONTKNOW
                 || f->GetFamilyType() == family)
                && (f->GetItalic() == ITALIC_DONTKNOW || italic == ITALIC_DONTKNOW
                    || f->GetItalic() == italic)
                && (f->GetWeight() == WEIGHT_DONTKNOW || weight == WEIGHT_DONTKNOW
                    || f->GetWeight() == weight)
                && (f->GetPitch() == PITCH_DONTKNOW || pitch == PITCH_DONTKNOW
                    || f->GetPitch() == pitch))
    ...
    if (selected != nullptr)
    {
        ...
        if (const void* data = graphics->GetEmbedFontData(selected, &size))
        {
            ...
            {
                osl::File file(url);
                if (file.open(osl_File_OpenFlag_Write | osl_File_OpenFlag_Create)
                    == osl::File::E_None)
                {
                    ...
                    while (written < totalSize && !error)
                    {
                        ...
                        switch (file.write(static_cast<const char*>(data) + written, size - written,
                                           nowWritten))
                        {
                            case osl::File::E_None:
                                written += nowWritten;

...
namespace vcl
{
bool HatchDrawableHelper::DrawDecomposedHatchLines(OutputDevice* pRenderContext,
                                                   ...
                                                   Hatch const& rHatch, bool bMtf)
{
    if (rPolyPolygon.Count())
    {
        ...
        if (rPolyPolygon.HasCurve())
        {
            ...
            DrawDecomposedHatchLines(pRenderContext, aPolyPoly, rHatch, bMtf);
        }
        else
        {
            ...
            DrawHatchLines(pRenderContext, rPolyPolygon, aRect, nWidth, rHatch.GetAngle(), bMtf);

            if ((rHatch.GetStyle() == HatchStyle::Double)
                || (rHatch.GetStyle() == HatchStyle::Triple))
            {
                // Double hatch
                DrawHatchLines(pRenderContext, rPolyPolygon, aRect, nWidth, rHatch.GetAngle() + 900,
                               bMtf);
...
void HatchDrawableHelper::DrawHatchLines(OutputDevice* pRenderContext,
                                         tools::PolyPolygon const& rPolyPolygon,
                                         tools::Rectangle const& rRect, long nDist,
                                         sal_uInt16 nAngle, bool bMtf)
{
    ...
    do
    {
        DrawHatchLine(pRenderContext, tools::Line(aPt1, aPt2), rPolyPolygon, bMtf);
...
void HatchDrawableHelper::DrawHatchLine(OutputDevice* pRenderContext, const tools::Line& rLine,
...
Size HatchDrawableHelper::GetHatchIncrement(long nDist, sal_uInt16 nAngle10)
{
...
Point HatchDrawableHelper::GetPt1(tools::Rectangle const& rRect, long nDist, sal_uInt16 nAngle10,
                                  Point aRef)
{
    long nAngle = nAngle10 % 1800;
    if (nAngle > 900)
...
Point HatchDrawableHelper::GetPt2(tools::Rectangle const& rRect, long nDist, sal_uInt16 nAngle10,
                                  Point aRef)
{
    long nAngle = nAngle10 % 1800;
    if (nAngle > 900)
...
Point HatchDrawableHelper::GetEndPt1(tools::Rectangle const& rRect, sal_uInt16 nAngle10)
{
    long nAngle = nAngle10 % 1800;
...
PointArray HatchDrawableHelper::GetHatchLinePoints(tools::Line const& rLine,
                                                   tools::PolyPolygon const& rPolyPolygon)
{
    double fX, fY;
    long nPCounter = 0;

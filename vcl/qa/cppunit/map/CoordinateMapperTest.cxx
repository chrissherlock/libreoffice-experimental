...
{
    // Ensure we are in Legacy Mode to reproduce old behavior
    ...
    // Legacy behavior: should collapse to Empty.
    tools::Rectangle aTinyRect(Point(10, 10), Size(2, 2));
    tools::Rectangle aResult = vcl::GeometryAdapter::Apply(aPlan, aTinyRect);

    CPPUNIT_ASSERT_MESSAGE("Rectangle should collapse to empty at 10% scale in Legacy Mode",
                           aResult.IsEmpty());
...
CPPUNIT_TEST_FIXTURE(CppUnit::TestFixture, testGhostPixelCollapse)
{
    ...
    // Input: Size(1, 1).
    // Math: 1 unit * 0.4 = 0.4 pixels.
    // Legacy behavior: 0.4 < 0.5 threshold triggers collapse to empty.
    tools::Rectangle aGhostRect(Point(10, 10), Size(1, 1));
    tools::Rectangle aResult = vcl::GeometryAdapter::Apply(aPlan, aGhostRect);
